...
docker run --name openapi_builder --rm -it \
        -w /gen \
        -e GEN_DIR=/gen \
        -e MAVEN_CONFIG=/tmp/.m2 \
        -e MAVEN_OPTS="-Dhttps.protocols=TLSv1.2 -Dmaven.repo.local=/tmp/.m2/repository -Dorg.slf4j.simpleLogger.log.org.apache.maven.cli.transfer.Slf4jMavenTransferListener=WARN -Dorg.slf4j.simpleLogger.showDateTime=true -Djava.awt.headless=true -Djacoco.skip=true" \
        -u "$(id -u):$(id -g)" \
        -v "${PWD}:/gen" \
        -v "${PWD}/CI/run-in-docker-settings.xml:/var/maven/.m2/settings.xml" \
        maven:3-eclipse-temurin-17 /bin/bash
        #-v "${maven_cache_repo}:/tmp/.m2/repository" \

# note : mvn -am -pl "modules/openapi-generator-cli" package -DskipTests
